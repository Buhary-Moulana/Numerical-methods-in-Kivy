...
class NRWindow(Screen):
    def nrfunction(self):
        ...
        for i in range(-100, 100, 1):
            if (
                (s.subs(x, i) > 0 and s.subs(x, i + 1) < 0)
                or (s.subs(x, i) < 0 and s.subs(x, i + 1) > 0)
                or (s.subs(x, i) == 0)
                or (s.subs(x, i + 1) == 0)
            ):
                a, b = i, i + 1
        ...
        self.ids.nrlabel1.text = (
            "Let our initial approximation of the root be x0 = " + str(x1)
        )
        for i in range(1, 6):
        ...
        if (s.subs(x, a) > 0 and s.subs(x, b) < 0) or (
            s.subs(x, a) > 0 and s.subs(x, b) == 0
        ):
            ...
            self.ids.nrlabelroot3.text = (
                "We conclude that atleast one root exists between "
                + str(a)
                + " and "
                + str(b)
            )
        ...
        if (s.subs(x, a) < 0 and s.subs(x, b) > 0) or (
            ...
            self.ids.nrlabelroot3.color = (50 / 255, 230 / 255, 98 / 255)
        self.ids.nrlabelfinal.text = "We conclude that our root is x = " + str(
            round(float(x1), 3)
        )
...
class SimpsonWindow(Screen):
    def simpsonfunctionsubmit(self):
        ...
        self.ids.simpson1resultlabel1.text = (
            "The Integral of f(x) by Simpson's 1/3rd rule is given by :"
        )
        ...
        self.ids.simpson1resultlabel7.text = (
            "The value of the Given Integral between the limits "
            + str(a)
            + " and "
            + str(b)
            + " is "
...
class SimpsonWindow2(Screen):
    def simpsonvaluenext(self):
        if self.ids.simpsonvaluenext.text != "Submit":
            ...
            if self.ids.simpsonvaluexinput.background_color == [1, 1, 1, 1]:
                ...
                n = int(s1[1])
                if n == n1:
                    simpsonxlist.append(float(self.ids.simpsonvaluexinput.text))
                    simpsonylist.append(float(self.ids.simpsonvalueyinput.text))
    ...
    # Calculating values of x and f(x)
            i = 0
            n=int(self.ids.simpsonvalueinput.text)
    ...
    # Calculating result
            ...
            while i< n:
                if i == 0 or i == n-1:
                    ...
                    res+= 4 * simpsonylist[i]
                    B+=simpsonylist[i]
                else:
                    res+= 2 * simpsonylist[i]
                    C+=simpsonylist[i]
                i+= 1
            res = res * (h / 3)
            I=round(res,5)
...
class RKWindow(Screen):
    def rkfunction(self):
        ...
        self.ids.rkresultlabel2.text = (
            "The value of y at x = " + str(x1) + " is " + str(round(y1, 5))
        )
